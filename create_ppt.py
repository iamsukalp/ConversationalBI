...
def create_pptx(slides_data):
    ...
    for slide_data in slides_data:
        ...
        textbox = slide.shapes.add_textbox(Inches(1), Inches(1.5), Inches(8.5), Inches(1))
        ...
        table = slide.shapes.add_table(rows+1, cols, Inches(1), Inches(3), Inches(8.5), Inches(2.5)).table
        ...
        for row_idx in range(rows):
            for col_idx in range(cols):
                table.cell(row_idx+1, col_idx).text = str(dataframe.iat[row_idx, col_idx])

        # Save the Plotly chart as an image and add it to the slide
        if chart is not None:
            import os
            # Create a temporary file inside the project folder
            with tempfile.NamedTemporaryFile(suffix=".png", dir=project_folder, delete=False) as tmpfile:
                chart_image_path = tmpfile.name
                pio.write_image(chart, chart_image_path)
                slide.shapes.add_picture(chart_image_path, Inches(1), Inches(5.5), Inches(8.5), Inches(3))
